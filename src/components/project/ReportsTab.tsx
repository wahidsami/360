...
export const ReportsTab: React.FC<ReportsTabProps> = ({ onRefresh, projectName }) => {
  ...
  const copy = useMemo(
    () => ({
      ...
      reportTypeLabel: isArabic ? 'Report' : 'Report',
    }),
    [isArabic],
  );
  ...
  const buildDefaultReportTitle = useCallback(
    () => `${(projectName || copy.fallbackProject).trim()} - Accessibility Report - ${format(new Date(), 'yyyy-MM-dd')}`,
    [copy.fallbackProject, projectName],
  );
